...
# Users can still opt-in to Cloudflare DNS using: hiarchy-setup-dns cloudflare
...
if [ -f /etc/systemd/resolved.conf ]; then
  ...
  if [ -f /etc/systemd/network/99-hiarchy-dns.network ]; then
    sudo rm -f /etc/systemd/network/99-hiarchy-dns.network
  ...
  echo "To use Cloudflare DNS, run: hiarchy-setup-dns Cloudflare"
fi
